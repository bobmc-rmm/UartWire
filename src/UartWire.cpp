...
#if USE_MCU==1 || USE_MCU==2
#define UWPORT  Serial1
#define UWLOG   Serial
#endif
#if USE_MCU==3
#define UWPORT  Serial  // swapped to 8266 
#define UWLOG   Serial1 // TX only
#endif
...
void uw_init( void ){
   uw_set_rate(0);
   uw_set_rate(9600);
   //Serial1.begin(115200);  while(!Serial1) delay(1);

#if USE_MCU==1
   UWPORT.begin(9600, SERIAL_8N1,ePIN_RX1,ePIN_TX1);  // esp32
#endif
   
#if USE_MCU==2
   UWPORT.begin(9600, SERIAL_8N1);
#endif

#if USE_MCU==3
   // note that USB Serial 8266 pins must be swapped
   // UART.begin(9600, SERIAL_8N1); // NA
   Serial.swap();  // move UART0 to UART2 (just swap pins)
#endif
...
static void uw_rx_flush(void){
   int status;
   do{ delay(1); status = UWPORT.read(); }while(status >= 0);
}
...
static void uw_set_rate(u32t rate){
   if( baudrate != rate ){
      baudrate = rate;
#if USE_MCU==1
      UWPORT.updateBaudRate( rate );  // esp32
#endif

#if USE_MCU==2
       UWPORT.begin(rate, SERIAL_8N1);  // meg2560
#endif
       
#if USE_MCU==3
      UWPORT.updateBaudRate( rate );  // 8266
#endif
   }
   uw_rx_flush();
}
